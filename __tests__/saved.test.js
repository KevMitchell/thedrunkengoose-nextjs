...
jest.mock('../utils/saved-drinks', () => ({
  getSaved: jest.fn((name) => name === 'drink2')
}))
...
const renderComponent = () => {
  const { container, queryByText, getByTestId } = render(<Saved />)
...
test('gets the saved drinks', () => {
  renderComponent()
  expect(getSaved).toHaveBeenCalledWith('drink1')
  expect(getSaved).toHaveBeenCalledWith('drink2')
})

test('renders saved section', () => {
